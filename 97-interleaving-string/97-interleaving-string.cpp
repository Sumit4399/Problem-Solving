...
public:
    ...
    bool memo(string &s1, string &s2, string &s3, 
              int l1, int l2, int l3, vector<vector<int>> &dp){
        ...
        //1st case- if curr char in s1=s2=s3
        if(l1>=0 && s1[l1]==s3[l3] && l2>=0 && s2[l2]==s3[l3])
            return dp[l1][l2]= (memo(s1, s2, s3, l1-1, l2, l3-1, dp) || 
                                memo(s1, s2, s3, l1, l2-1, l3-1, dp));
        
        //2nd case- if curr char in s1==s3
        else if(l1>=0 && s1[l1]==s3[l3])
            return memo(s1, s2, s3, l1-1, l2, l3-1, dp);
        
        //3rd case- if curr char in s2==s3
        else if(l2>=0 && s2[l2]==s3[l3])
            return memo(s1, s2, s3, l1, l2-1, l3-1, dp);
        
        //if curr didn't match
        else
            return false;
    }
    ...
    bool isInterleave(string s1, string s2, string s3) {
        ...
        vector<vector<int>>dp(s1.size()+1, (vector<int>(s2.size()+1, -1)));
        return memo(s1, s2, s3, s1.size()-1, s2.size()-1, s3.size()-1, dp);
    }
};
